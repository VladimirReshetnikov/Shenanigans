...
| --- | --- | --- |
| [`Lean/`](Lean/) | Lean 4 | The name-keyed accelerator family (`Nat.add`, `Nat.beq`, the nine free names under `Init.Prelude`, `Lean.reduceBool`, and string-literal fabrication), and **three defects live on every released toolchain**: the universe spelling in [`Universes/`](Lean/Universes/), and the two of 2026-08-17/18 in [`DefEq/`](Lean/DefEq/) — a definitional-equality *cache* whose transitive closure lets a recursor's type disagree with its computation rule, and an `is_prop` that answers "no" for a term it should reject. The module boundary that loses `partial` in [`ModuleSystem/`](Lean/ModuleSystem/) was a fourth until `v4.33.0` shipped [lean4#14609](https://github.com/leanprover/lean4/pull/14609); it is now a **regression witness** — measured 2026-08-20 as building on its pinned `v4.32.2` and refused by `v4.33.0` with `(kernel) invalid declaration, it uses unsafe declaration 'partialFalse'`. Plus `Expr.proj` index truncation and level-normalization incompleteness. With [`Comparator/`](Lean/Comparator/), which shows the Lean FRO's proof judge accepting one of them. |
