...
> them. `ImaxPropSpelling.lean` and the three `DefEq/` exhibits are ordinary
...
## The most severe failure mode: the kernel manufactures an inhabitant of an empty type

`StringLitEmptyInhabitant.lean` is qualitatively different from the arithmetic
...
| [`Accelerators/StringLitEmptyInhabitant.lean`](Accelerators/StringLitEmptyInhabitant.lean) | **Most severe failure mode.** Nothing "disagrees" with anything: expanding a *string literal* makes the kernel assemble a term by name and feed it to a recursor rule **without type-checking it**, manufacturing an inhabitant of `Empty` — a type with no constructors. Trigger is a bare `"a"`; no `Nat`, numerals, `OfNat`, `List` or `Char` needed. | accepts |
...
| [`ModuleSystem/`](ModuleSystem/) | **Not `prelude`, not even a kernel defect; now a regression witness.** Measured 2026-08-20: builds on its pinned `v4.32.2`, refused by `v4.33.0` (`(kernel) invalid declaration, it uses unsafe declaration 'partialFalse'`), since `v4.33.0` carries [#14609](https://github.com/leanprover/lean4/pull/14609). A two-module Lake package: a `partial` definition of type `False` crosses a module boundary as a *safe* axiom stub (lean4#14609), so `public theorem Paradox : False` is accepted from a safe declaration. **`#print axioms` reports nothing** — the second Lean route invisible to the audit, and the first that needs no option. | **rejects** — and it is the only exhibit here that `leanchecker` catches, because the defect is in the frontend rather than in the kernel it shares |
| [`Universes/ImaxPropSpelling.lean`](Universes/ImaxPropSpelling.lean) | **Not `prelude`; now a regression witness.** Measured 2026-08-20 across three toolchains: exit 0 on `v4.32.2`, and refused by both `v4.33.0` and `v4.34.0-rc1` with `(kernel) invalid projection` — `v4.33.0` shipped the July wave ([#14615](https://github.com/leanprover/lean4/pull/14615)'s `normalizes_to_zero`). Read the "accepts" verdict below as ending at `v4.32.2`. `theorem Paradox : False` from lean4#14613: `is_prop` compares the sort spelling against `Sort 0` syntactically, so one inductive is a proposition for proof irrelevance and not one for `infer_proj`. Uses only genuine core constants, so §2.3's prelude policy does not cover it. Two spellings of zero (`imax 1 0` and `max 0 0`), each with its own `False`. | **accepts** — it shares Lean's kernel. `nanoda` rejects it. Verified by [`Universes/verify.ps1`](Universes/verify.ps1) |
| [`Universes/MutualResultLevel.lean`](Universes/MutualResultLevel.lean) | The *inheritance* step measured alone, with the order reversal as its control. `m_result_level` comes from the first type of a mutual block; that is how the contents above gets declared at all, and it is unchanged on `master`. No `False`. | n/a |
...
| [`Controls/NegativeControl.lean`](Controls/NegativeControl.lean) | Control. `set_option debug.skipKernelTC true` places a blatantly ill-typed `illTyped : False` into an `.olean`. It too builds with exit 0 and reports no axioms. | **rejects** — which is what makes acceptance of the others meaningful |
