...
| [`KernelDefects/`](KernelDefects/) | Genuine implementation defects. Lean: the name-keyed `Nat`/`String`/`reduceBool` accelerator family, `Expr.proj` index truncation, and the **one** defect here that is live on the current stable release — a definitional-equality *cache* whose transitive closure lets a recursor's type disagree with its computation rule ([#14806](https://github.com/leanprover/lean4/pull/14806)), and an `is_prop` that answers "no" for a term it should reject, skipping the proof-irrelevance guard on projections ([#14807](https://github.com/leanprover/lean4/pull/14807)), and a `class inductive` recursor that keeps a dependency its module's exported view drops, letting a downstream module rebind that global name ([#14875](https://github.com/leanprover/lean4/issues/14875), reported 2026-08-21 and **open** — the only one of the three `leanchecker` catches) — with controls and a `leanchecker` verdict for each. A universe spelling the kernel reads two ways and a module boundary that loses `partial` were counted here until `v4.33.0` shipped the July wave; both are now **regression witnesses**, measured across three toolchains. [#14838](https://github.com/leanprover/lean4/pull/14838), a 32-bit reference-count overflow in the runtime, has no exhibit and cannot have one — it needs 12–18GB of RAM to reproduce; what can be measured about it is in [`Audits/Lean/Runtime/`](Audits/Lean/Runtime/), and it is CATALOG §2.7. **`v4.33.1`, released 2026-08-21, fixed #14806, #14807, #14808 and #14838 at once**, so the `DefEq/` pair joined the regression witnesses the same day they were catalogued as live. Rocq: four fixed guard-checker and module-system defects kept as regression witnesses, plus **three live routes** — [rocq#22287](https://github.com/rocq-prover/rocq/issues/22287), a `False` with a clean `Print Assumptions` that `coqchk` still catches; [rocq#21839](https://github.com/rocq-prover/rocq/issues/21839) and **six more from the 2026-08-20 wave**, each a closed flag-free `False` that **`Print Assumptions` and `coqchk` both miss** and that escapes through a plain `Require` — `AuditBlindSextet.v` is the one file that `Require`s six of them at once and reports six clean audits; and [rocq#22352](https://github.com/rocq-prover/rocq/issues/22352), the first defect in this catalog that is in a **checker** rather than a kernel — `rocqchk -bytecode-compiler yes` reports `Modules were successfully checked` over a `False`, while plain `rocqchk` rejects the same bytes. |
...
pwsh KernelDefects/Lean/ModuleSystem/classrec/verify.ps1  # lean4#14875 behaved as documented on: v4.33.1
pwsh KernelDefects/Coq/verify.ps1          # All 42 Coq exhibits behaved as documented.
