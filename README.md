...
| [`KernelDefects/`](KernelDefects/) | Genuine implementation defects. Lean: the name-keyed `Nat`/`String`/`reduceBool` accelerator family, `Expr.proj` index truncation, definitional-equality history dependence, and the universe-spelling defect that is **live on every released toolchain** — with controls and a `leanchecker` verdict for each. Rocq: four fixed guard-checker and module-system defects, kept as regression witnesses. |
