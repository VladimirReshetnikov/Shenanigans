...
| --- | --- | --- |
| **The kernel applies quotient computation rules to constants the USER declared** — a new route to `False`, found here 2026-08-21 | `Kernel.Environment.quotInit` gates `quot_reduce_rec`, and its docstring promises that when set, "the type checker can assume that the `Quot` declarations in the environment have indeed been added by the kernel and **not by the user**". Nothing establishes that: `Lean/Environment.lean` sets `quotInit := !imports.isEmpty`. A `prelude` chain that never reaches `Init.Prelude` has non-empty imports and has never run `init_quot`, so the flag is true while `Quot`/`Quot.mk`/`Quot.lift` are **free names** — measured. `quot_reduce_rec` (`src/kernel/quot.h`) then fires on names and arity with **no type check at all**, returning `mk_app(f, app_arg(mk))`. Declaring `Quot A r := MyUnit` (so structure eta equates any two `Quot.mk`s) plus an **`opaque`** `Quot.mk` (so `whnf` cannot unfold the head) gives two terms that differ only in a definitionally-equal argument and reduce to **different constructors**; congruence via `MyEq.rec` and a transport finish it. Nothing in `environment::add_*` refuses to declare these names — `add_quot`'s `check_name` (#14632) guards only the opposite direction. | **artifact** — [`Quotient/`](KernelDefects/Lean/Quotient/). **Verified here** on `v4.33.0`, `v4.33.1` and `v4.34.0-rc2`: one self-contained term through `addDeclCore` into a **fresh** environment, **ACCEPTED** on all three, with the renamed control **rejected** on all three; `MyFalse` has zero constructors and the axiom closure is **empty**. Needs `prelude`, so §2.3's policy may cover it — but the defect is a **derived flag** standing in for a fact, not a name-keyed accelerator, and the elaborator disagrees with the kernel about the same terms |
